...
homeServer <- function(id, shared_store, shared_rx) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$load, {
      ...
      shared_store$original_data[["datasetA"]] <- data.table::copy(datasetA_dt)
      shared_store$original_data[["datasetB"]] <- data.table::copy(datasetB_dt)

      stopifnot(
        !identical(
          lobstr::obj_addr(shared_store$data_for_data[["datasetA"]]),
          lobstr::obj_addr(shared_store$original_data[["datasetA"]])
        )
      )
