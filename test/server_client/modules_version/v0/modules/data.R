  ...
  moduleServer(id, function(input, output, session) {
    ...
    render_tbl_once <- function() {
      ...
      output$tbl <- renderDT({
        ...
        ", rid_idx0, id_snapshot, id_edit_row, id_edit_col))
      ...
      # Initial push (safe; dt_ready gate will handle subsequent updates as needed)
      isolate({
        view_df <- apply_slice(df_full0, slice_pct())
        cat(sprintf("[Data] Initial push rows=%d cols=%d\n", nrow(view_df), ncol(view_df)))
        replaceData(proxy, data = view_df, resetPaging = TRUE, rownames = FALSE, clearSelection = "none")
      })
    }
    ...
    # ---- Snapshot logging; flip dt_ready on first init/draw ----
    ...
    observeEvent(input$tbl_snapshot, ignoreInit = FALSE, {
      ...
      if (!is.null(snap$orderAll)) cat("[orderAll (ColReorder, 0-based, all cols)] ", paste(as.integer(snap$orderAll), collapse = ","), "\n", sep = "")
      if (!is.null(snap$dataSrc))  cat("[dataSrc] ", paste(as.integer(snap$dataSrc), collapse = ","), "\n", sep = "")
      cat("===== end tbl_snapshot =====\n")

      # Consider the table 'ready' after first init/draw event
      if (!dt_ready() && (snap$reason %in% c("init", "draw"))) {
        dt_ready(TRUE)
        cat("[Data] DT is ready in browser; enabling replaceData updates.\n")
        # Push a refresh now that DT is ready (handles the initial case if needed)
        update_view(resetPaging = TRUE)
      }
    })
    ...
    observeEvent(input$tbl_cell_edit, ignoreInit = TRUE, {
      ...
      # Allowlist example (adjust as needed)
