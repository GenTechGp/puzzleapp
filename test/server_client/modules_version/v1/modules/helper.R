...
make_boundary_table <- function(x,
  ...
  # If nothing was added (no numeric/factor), keep template
  boundary <- if (length(rows_list) == 0) template else data.table::rbindlist(rows_list, use.names = TRUE, fill = TRUE)
