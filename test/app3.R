...
# Design decision: server-side authoritative column visibility
# Fix applied: removed incorrect server = TRUE arguments passed to datatable(...)
#              use renderDT(..., server = TRUE) when server-side processing is desired.
#
...
dataModuleUI <- function(id) {
  ns <- NS(id)
  tagList(
    uiOutput(ns("col_controls")),    # dynamic show/hide controls (checkboxes + select all/none/preferred)
...
dataModuleServer <- function(id, shared_data) {
  moduleServer(id, function(input, output, session) {
        ...
        tagList(
          fluidRow(
            ...
            column(4, style = "padding-top: 25px;",
                   ...
                   " | ",
                   actionLink(ns("select_preferred"), "Select preferred")
            )
          ),
          tags$div(style = "font-size:90%; color:#666;", "Visible columns are authoritative server-side (checkboxes).")
        )
      }
    })

    # Select all / none / preferred actions
    ...
    observeEvent(input$select_preferred, {
      df <- shared_data$data
      pref <- shared_data$preferred_cols
      if (is.null(df)) {
      ...
      if (is.null(pref) || length(pref) == 0) {
        showNotification("No preferred columns set.", type = "warning")
        return()
      }
      pref_valid <- intersect(pref, colnames(df))
      if (length(pref_valid) == 0) {
        showNotification("None of the preferred columns exist in the loaded data.", type = "warning")
        return()
      }
      updateCheckboxGroupInput(session, "cols", selected = pref_valid)
    })

    # When data is newly loaded, ensure the checkboxGroup matches the columns.
    # Use preferred_cols as initial selection if available & valid, otherwise select all.
    observeEvent(shared_data$data, {
      ...
      if (is.null(df)) {
        ...
        if (!is.null(pref) && length(pref) > 0) {
          ...
          updateCheckboxGroupInput(session, "cols", choices = cols, selected = initial_sel)
        } else {
          updateCheckboxGroupInput(session, "cols", choices = cols, selected = cols)
        }
      }
    }, ignoreNULL = FALSE)
    ...
    output$data_table <- renderDT({
      ...
      if (is.null(tbl)) {
        ...
        # compute display order: preferred (valid) first, then remainder
...
# ---------------------------
ui <- fluidPage(
  titlePanel("3-Tab Shiny App (path input, server-side column controls)"),
  tabsetPanel(
    id = "tabs",
