...
svs_vcf <- system(sprintf("ls -tr %s/Variant_calls/jasmine/*/%s/*/*.*.all.trio.sorted.gatk.vcf.gz | tail -n1", project_dir, sample), intern = TRUE)
...
svs_processed_data <- consequence_data_dcast[, .(
  ...
  AD_1, AD_2, AD_3, DP_1, DP_2, DP_3, GT_1, GT_2, GT_3
)]
