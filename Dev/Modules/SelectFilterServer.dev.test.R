    ...
    observeEvent(input$apply_filter, {
      total_time <- system.time({
        ...
        finaltable_time <- system.time({
          ...
          filtered_dataset <- data.table(PRIORITY = 0, dataset[ID %in% final_filtered_ids])
          
          if (length(panelapp_filter) > 0) {

            if (nrow(filtered_dataset) > 0) {
              print("here")
              filtered_dataset <- merge(filtered_dataset,genes[,.(ID,PANEL_APP)],by="ID")
            } else {
    ...
    observe({
      ...
      initialisation_time <- system.time({
        if (initialisation() == TRUE) {
          #print("here")
          updateSelectInput(session, "inheritance", selected = "Homozygous Recessive")
          #updateSelectInput(session, "inheritance", selected = "Compound Heterozygous")
          #updateSelectInput(session, "panelapp", selected = "Mendeliome")
