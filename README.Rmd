...
1. Visit [NCI Batch Connect Dashboard](https://are.nci.org.au/pun/sys/dashboard/batch_connect/sessions)
2. Launch an RStudio job. Remember to add `gdata/if89` to the storage parameter. Example parameters:
``` 
  mem=64GB
  ncpus=4
  jobfs=10GB
...
- igvShiny is not available on Bioconductor for R 4.3 (Bioconductor 3.18); install it from GitHub instead.
- Then install puzzleapp from a local checkout after lowering the R version requirement in DESCRIPTION.
