  ...
  div(class = "header",
   ...
   tabsetPanel(
    tabPanel("Home",
      ...
      qcPlotsUI(sprintf("%s-tab6",sample),"QC Plots", ifelse(!is.null(coverage_data),TRUE,FALSE), ifelse(!is.null(somalier),TRUE,FALSE), ifelse(nrow(processed_data[CATEGORY=="SNV & Indel"])==0,TRUE,FALSE))
  )
)
