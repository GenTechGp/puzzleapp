...
dataset_editable_cols <- function(dataset_key, df_full) {
  ...
  intersect(c("Sepal.Width", "NOTES", "PRIORITY"), names(df_full))
}
...
dataset_after_edit <- function(dataset_key, df_full, row_idx, colname, old_value, new_value) {
  # If PRIORITY edited, (re)derive PRIORITYFlag
  if (identical(colname, "PRIORITY") &&
      "PRIORITY" %in% names(df_full)) {

    # Ensure PRIORITYFlag column exists
    ...
    df_full[row_idx, PRIORITYFlag := flag]
  }
